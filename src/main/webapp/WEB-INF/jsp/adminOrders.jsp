...
<form id="form" action="requestOrders.do" method="post">
    检索条件<br/>
    用户名<input type="text" name="userId" id="userIdField" onchange="setPageOne()" <c:if
        test="${!empty userId}"> value=${userId}  </c:if>/>
...
<c:choose>
    ...
    <c:otherwise>
        ...
        <table border="2" style="table-layout:fixed">
            ...
            <c:forEach items="${dataPage.data}" var="order">
                <tr>
                    <td>${order.id}</td>
                    <td>${order.userId}</td>
                    <td>${order.machineId}</td>
                    <td><fmt:formatDate value="${order.time}" pattern="yyyy年MM月dd日 HH时mm分ss秒"/></td>
                    <td><a href="deleteOrder.do?orderId=${order.id}" onClick="return confirm('确定删除?');">[删除]</a></td>
                </tr>
            </c:forEach>
